...
RELEASEFILE="https://github.com/pade/padecore/releases/download/v0.12.1.5/padecore-0.12.1.5-linux64.tar.gz"
SENTINELGIT="https://github.com/pade/sentinelLinux.git" # leave empty if coin has no sentinel

daemon="paded"
cli="pade-cli"
stopcli="stop"
archive_path="padecore-0.12.1.5/bin/"
core_dir=".padecore"
config_path="$core_dir/pade.conf"
node_user="pade"
mainnet="13783"
...
function stop_services {
	...
	systemctl stop paded-reindex
	systemctl stop paded
	sleep 3
	killall -9 paded 2>/dev/null
}
...
function install_cronjob {
	...
	crontab -l | { cat; echo "*/15 * * * * (( (\$(curl -s http://explorer.pade.com/api/getblockcount) - \$(pade-cli getblockcount)) > 10 )) && systemctl restart paded-reindex" ;} | crontab -
}
