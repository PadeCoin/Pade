...
const char * const BITCOIN_CONF_FILENAME = "pade.conf";
const char * const BITCOIN_PID_FILENAME = "paded.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pade" is a composite category enabling all Pade-related debug output
            if(ptrCategory->count(string("pade"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PadeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PadeCore
    // Mac: ~/Library/Application Support/PadeCore
    // Unix: ~/.padecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PadeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PadeCore";
#else
    // Unix
    return pathRet / ".padecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pade.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pade.conf
